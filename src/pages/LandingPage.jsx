...
const DISCIPLINAS = [
  ...
  { nombre: 'JUDO',             img: imgJudo,     color: '#f97316', desc: 'Técnica y proyección'             },
  { nombre: 'TAEKWONDO',        img: imgTaekwondo,color: '#eab308', desc: 'Velocidad y precisión'            },
  { nombre: 'BOXEO',            img: imgBoxeo,    color: '#22c55e', desc: 'Resistencia explosiva'            },
  { nombre: 'DEFENSA PERSONAL', img: imgDefensa,  color: '#3b82f6', desc: 'Combate real y táctico'           },
]
    ...
    <div style={{ position: 'relative', width: 160, height: 160, margin: '0 auto 32px' }}>
      {PIECES.map((p, i) => (
        <div key={i} style={{
          ...
          color: '#c9a227', pointerEvents: 'none', zIndex: 1,
        }}>{p.symbol}</div>
      ))}
      <div style={{
        position: 'absolute', top: '50%', left: '50%',
        ...
        <svg viewBox="0 0 100 120" width="100">
          ...
          <path d="M50 5 L92 22 L92 65 Q92 100 50 115 Q8 100 8 65 L8 22 Z" fill="url(#sg2)" stroke="#c9a227" strokeWidth="2.5"/>
          <path d="M50 14 L83 28 L83 65 Q83 95 50 108 Q17 95 17 65 L17 28 Z" fill="none" stroke="rgba(201,162,39,0.35)" strokeWidth="1"/>
          <line x1="50" y1="28" x2="50" y2="95" stroke="#c9a227" strokeWidth="2" opacity="0.8"/>
          <line x1="25" y1="55" x2="75" y2="55" stroke="#c9a227" strokeWidth="2" opacity="0.8"/>
          <text x="50" y="62" textAnchor="middle" fill="#c9a227" fontSize="20" fontFamily="serif">⚔</text>
        </svg>
      </div>
    </div>
  )
...
function CustomCursor() {
  ...
  return <>
    <div ref={ringRef} style={{ position:'fixed',top:0,left:0,zIndex:9998,pointerEvents:'none',width:28,height:28,borderRadius:'50%',border:'1.5px solid #c9a227',transition:'width 0.15s,height 0.15s',mixBlendMode:'difference' }}/>
    <div ref={dotRef}  style={{ position:'fixed',top:0,left:0,zIndex:9998,pointerEvents:'none',width:6,height:6,borderRadius:'50%',background:'#c9a227' }}/>
  </>
}
    ...
    <div style={{ background: '#060a06', overflowX: 'hidden', fontFamily: 'Georgia, serif' }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;cursor:none!important}
        ...
        @keyframes glowPulse{ 0%,100%{box-shadow:0 0 15px #c9a227,0 0 40px rgba(201,162,39,0.25)} 50%{box-shadow:0 0 30px #c9a227,0 0 80px rgba(201,162,39,0.45)} }
        ...
        .shimmer{
          background:linear-gradient(90deg,#c9a227 0%,#f0d060 30%,#fff8dc 50%,#f0d060 70%,#c9a227 100%);
          background-size:200% auto;
          -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;
          animation:shimmer 3s linear infinite;
        }
        .btn-primary{
          background:linear-gradient(135deg,#8b0000,#c9a227,#8b0000);background-size:200% 100%;
          border:2px solid #c9a227;border-radius:3px;padding:18px 52px;color:white;
        ...
        .btn-outline{
          background:transparent;border:1px solid rgba(201,162,39,0.4);border-radius:3px;
        ...
        .btn-outline:hover{border-color:#c9a227;color:#c9a227;background:rgba(201,162,39,0.05)}
        ...
        .slide-btn{width:52px;height:52px;border-radius:50%;background:rgba(201,162,39,0.1);border:1px solid rgba(201,162,39,0.5);color:#c9a227;font-size:20px;display:flex;align-items:center;justify-content:center;transition:all 0.3s}
        .slide-btn:hover{background:rgba(201,162,39,0.2);border-color:#c9a227;transform:scale(1.1)}
        ...
        .stat-card:hover{transform:translateY(-8px) scale(1.03);border-color:#c9a227!important}
      ...
      {introPhase !== 'done' && (
        <div style={{ position:'fixed',inset:0,zIndex:9999,pointerEvents: introPhase==='reveal' ? 'none' : 'all' }}>
          <div style={{
            ...
            background:'linear-gradient(135deg,#0d1a0f 0%,#1a3d26 50%,#0d1a0f 100%)',
            ...
            <div style={{ textAlign:'right' }}>
              ...
              <p style={{ fontFamily:'Cinzel,serif',color:'#c9a227',fontSize:11,letterSpacing:'0.4em',animation:'curtainText 1s ease-out 0.5s both' }}>EJÉRCITO DE</p>
          ...
          <div style={{
            ...
            background:'linear-gradient(225deg,#0d1a0f 0%,#1a3d26 50%,#0d1a0f 100%)',
            ...
            <div>
              <p style={{ fontFamily:'Cinzel,serif',color:'#c9a227',fontSize:11,letterSpacing:'0.4em',animation:'curtainText 1s ease-out 0.6s both' }}>UNIDAD DE</p>
          ...
          <div style={{
            ...
            width:80,height:80,borderRadius:'50%',border:'2px solid #c9a227',
            display:'flex',alignItems:'center',justifyContent:'center',
            fontSize:36,background:'rgba(6,10,6,0.8)',
      ...
      <nav style={{
        ...
        background: scrollY > 60 ? 'rgba(6,10,6,0.96)' : 'transparent',
        backdropFilter: scrollY > 60 ? 'blur(16px)' : 'none',
        borderBottom: scrollY > 60 ? '1px solid rgba(201,162,39,0.18)' : 'none',
        transition:'all 0.4s',
      }}>
        <div style={{ display:'flex',alignItems:'center',gap:12 }}>
          <div style={{ width:36,height:36,borderRadius:'50%',border:'1.5px solid #c9a227',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16,animation:'glowPulse 3s infinite' }}>⚔️</div>
          <div>
            <p style={{ fontFamily:'Cinzel,serif',color:'#c9a227',fontSize:9,letterSpacing:'0.2em' }}>UNIDAD DE</p>
        ...
        <div style={{ display:'flex',gap:12,alignItems:'center' }}>
          <button onClick={toggleMusic} style={{
            background: playing ? 'rgba(201,162,39,0.15)' : 'transparent',
            border:'1px solid rgba(201,162,39,0.35)',borderRadius:6,padding:'7px 14px',
            color:'#c9a227',fontFamily:'Cinzel,serif',fontSize:9,letterSpacing:'0.15em',
      ...
      <section id="inicio" style={{ position:'relative', height:'100vh', overflow:'hidden' }}>
        ...
        <div style={{
          ...
          background:`linear-gradient(135deg, rgba(6,10,6,0.82) 0%, rgba(6,10,6,0.55) 50%, rgba(6,10,6,0.88) 100%)`,
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{
              fontFamily:'Cormorant Garamond,Georgia,serif',
              color:'rgba(255,255,255,0.42)', fontSize:17, lineHeight:1.8,
        ...
        <div style={{
          ...
          <p style={{ fontFamily:'Cinzel,serif', color:'rgba(201,162,39,0.45)', fontSize:8, letterSpacing:'0.4em' }}>EXPLORAR</p>
          <div style={{ width:1, height:44, background:'linear-gradient(to bottom,#c9a227,transparent)', animation:'scan 1.8s ease-in-out infinite' }}/>
        </div>
      </section>
      ...
      <section style={{
        ...
        background:'linear-gradient(180deg,#060a06 0%,#0d1a0f 100%)', overflow:'hidden',
      }}>
        <div style={{ position:'absolute',top:0,left:0,right:0,height:1,background:'linear-gradient(90deg,transparent,#c9a227,transparent)' }}/>

        <div ref={statsRef} style={{ textAlign:'center', marginBottom:60 }}>
          <PuzzlePieces visible={statsVis} />
          <p style={{
            fontFamily:'Cinzel,serif', color:'#c9a227', fontSize:10, letterSpacing:'0.5em', marginBottom:8,
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))', gap:20, maxWidth:1000, margin:'0 auto' }}>
          {STATS.map((s, i) => (
            <div key={i} className="stat-card" style={{
              ...
              border:'1px solid rgba(201,162,39,0.18)', borderRadius:3,
              background:'rgba(201,162,39,0.025)', position:'relative', overflow:'hidden',
              ...
              <div style={{ position:'absolute',top:0,left:0,width:18,height:18,borderTop:'2px solid #c9a227',borderLeft:'2px solid #c9a227' }}/>
              <div style={{ position:'absolute',bottom:0,right:0,width:18,height:18,borderBottom:'2px solid #c9a227',borderRight:'2px solid #c9a227' }}/>
      ...
      ══════════════════════════════════════════ */}
      <section id="disciplinas" style={{ position:'relative', padding:'110px 6vw', background:'#060a06', overflow:'hidden' }}>
        <div style={{
        ...
        <div ref={discRef} style={{ textAlign:'center', marginBottom:64 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:16, marginBottom:18 }}>
            <div style={{ width:70, height:1, background:'linear-gradient(to right,transparent,#c9a227)' }}/>
            <span style={{ fontFamily:'Cinzel,serif', color:'#c9a227', fontSize:10, letterSpacing:'0.5em' }}>ARTE Y COMBATE</span>
            <div style={{ width:70, height:1, background:'linear-gradient(to left,transparent,#c9a227)' }}/>
          </div>
          <h2 style={{
      ...
      ══════════════════════════════════════════ */}
      <section id="unidad" style={{ position:'relative', padding:'120px 6vw', background:'linear-gradient(180deg,#060a06 0%,#0d1a0f 60%,#060a06 100%)', overflow:'hidden' }}>
        <div style={{ position:'absolute',inset:0,backgroundImage:`repeating-linear-gradient(-45deg,rgba(201,162,39,0.03) 0px,rgba(201,162,39,0.03) 1px,transparent 1px,transparent 40px)` }}/>

        <div style={{ maxWidth:1100, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr', gap:80, alignItems:'center' }}>
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:22 }}>
              <div style={{ width:40, height:2, background:'#c9a227' }}/>
              <p style={{ fontFamily:'Cinzel,serif', color:'#c9a227', fontSize:10, letterSpacing:'0.4em' }}>NUESTRA MISIÓN</p>
            </div>
            ...
            <p style={{ fontFamily:'Cormorant Garamond,serif', color:'rgba(255,255,255,0.6)', fontSize:19, lineHeight:1.85, fontStyle:'italic', marginBottom:40 }}>
          ...
          <div ref={ctaRef} style={{ display:'flex', flexDirection:'column', gap:14 }}>
            {[
              { n:'01', t:'EVALUACIÓN CONTINUA',  d:'Seguimiento permanente del rendimiento de cada efectivo',                        c:'#c9a227' },
              { n:'02', t:'ANÁLISIS 3D AVANZADO', d:'Radar de competencias en tiempo real con visualización tridimensional',           c:'#4ade80' },
              { n:'03', t:'REPORTES PDF',          d:'Informes individuales y generales descargables al instante',                     c:'#60a5fa' },
              { n:'04', t:'MOTOR DE IA',           d:'Predicción de rendimiento y alertas automáticas por inteligencia artificial',    c:'#f97316' },
            ].map((item, i) => (
      ...
      {/* ══ FOOTER ══ */}
      <footer style={{
        borderTop:'1px solid rgba(201,162,39,0.18)', padding:'28px 6vw', background:'#060a06',
